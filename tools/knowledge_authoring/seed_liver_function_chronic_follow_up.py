...
def fragment():
    e = [
        ...
        Q("patient.alcohol.use_status", "Alcohol Use Status", "coded", "alcohol-status", "현재 술을 마시는지, 과거에는 마셨지만 지금은 끊었는지, 평생 마시지 않았는지 알려주세요.", 142, [G["common"], G["cause"]], R, allowed_values=["current", "former", "never"], reuse_existing=True),
        Q("patient.alcohol.beverage_types", "Alcoholic Beverage Types", "coded_or_string", "alcohol-beverage-types", "주로 마시거나 마셨던 술 종류는 무엇인가요? 보기에 없으면 직접 입력해 주세요.", 141, [G["common"], G["cause"]], R, allowed_values=["soju", "beer", "makgeolli", "wine", "spirits", "other"], reuse_existing=True),
        Q("patient.alcohol.frequency", "Alcohol Drinking Frequency", "string", "alcohol-frequency", "평균적으로 얼마나 자주 술을 마시거나 마셨나요? 예: 주 2회, 월 1회, 거의 매일", 140, [G["common"], G["cause"]], R, reuse_existing=True),
        Q("patient.alcohol.amount_per_occasion", "Alcohol Amount per Occasion", "string", "alcohol-amount", "한 번 마실 때 보통 술 종류별로 얼마나 마시거나 마셨나요? 예: 소주 1병, 맥주 500mL 2잔", 139, [G["common"], G["cause"]], R, reuse_existing=True),
        Q("liver.alcohol_heavy_episode_pattern", "Heavy Alcohol Episode Pattern", "string", "alcohol-heavy-episode", "평소보다 많은 양을 한 번에 마시는 일이 있다면 양과 빈도를 알려주세요. 없으면 '없음'이라고 답해 주세요.", 138, [G["common"], G["cause"]], R),
        Q("liver.alcohol_last_use_timing", "Last Alcohol Use Timing", "date_or_period", "alcohol-last-use", "마지막으로 술을 마신 때는 언제인가요? 정확한 날짜를 모르면 대략적인 시점을 알려주세요.", 137, [G["common"], G["cause"]], R),
        Q("liver.alcohol_recent_reduction_or_cessation", "Recent Alcohol Reduction or Cessation", "boolean", "alcohol-reduction", "최근 평소보다 술을 크게 줄이거나 끊었나요?", 136, [G["common"], G["cause"]], R),
        Q("liver.alcohol_reduction_or_cessation_details", "Alcohol Reduction or Cessation Details", "string", "alcohol-reduction-details", "술을 줄이거나 끊기 시작한 시점과 이전·현재 음주량의 변화를 알려주세요.", 135, [G["common"], G["cause"]], R),
    ...
    refresh.update({"last_assessed_at": "2026-07-30", "next_monitor_at": "2026-08-06", "next_full_review_at": "2027-01-26"})
...
def completion(f):
    ...
    alcohol_details = ["patient.alcohol.beverage_types", "patient.alcohol.frequency", "patient.alcohol.amount_per_occasion", "liver.alcohol_heavy_episode_pattern", "liver.alcohol_last_use_timing", "liver.alcohol_recent_reduction_or_cessation"]
    p["required_facts"]["routine"], p["conditional_required_facts"] = common, [
        {"selector_fact": "liver.primary_group", "cases": cases},
        {"selector_fact": "patient.alcohol.use_status", "cases": {"current": alcohol_details, "former": alcohol_details, "never": []}},
        {"when": {"fact": "liver.alcohol_recent_reduction_or_cessation", "equals": True}, "required_facts": ["liver.alcohol_reduction_or_cessation_details"]},
    ]
    return p
...
def source_docs():
    defs = [
        ("source.nice.ng50.cirrhosis.2023", "NICE", "Cirrhosis in over 16s: assessment and management", "NG50; updated-2023-09-08", "https://www.nice.org.uk/guidance/ng50/chapter/Recommendations", "nice_guidance", "2026-07-30", "current_official_source_confirmed_no_clinical_change", ["Cirrhosis assessment includes cause, decompensation, varices, ascites, encephalopathy, medicines and hepatocellular carcinoma surveillance.", "Alcohol misuse and quantified weekly alcohol exposure remain relevant cirrhosis-risk context.", "Gastrointestinal bleeding, acute confusion, infected ascites and respiratory compromise require urgent escalation."]),
        ("source.nice.ng49.nafld.2024", "NICE", "Non-alcoholic fatty liver disease: assessment and management", "NG49; links-updated-2025-03", "https://www.nice.org.uk/guidance/ng49", "nice_guidance", "2026-07-30", "official_metadata_change_reviewed_links_only_no_clinical_import", ["Steatotic liver disease may be asymptomatic and requires metabolic risk, fibrosis assessment and imaging context rather than liver enzymes alone.", "The official overview reports a March 2025 link update and an update in development; no new clinical recommendation was imported in this refresh."]),
        ("source.nice.cg165.hepatitis-b", "NICE", "Chronic hepatitis B: diagnosis and management", "CG165; updated-2017-10-20", "https://www.nice.org.uk/Guidance/CG165/chapter/recommendations", "nice_guidance", "2026-07-30", "current_official_source_confirmed_no_clinical_change", ["Viral status, treatment, pregnancy, fibrosis, decompensation and HCC surveillance are relevant follow-up domains.", "Alcohol intake and treatment adherence remain relevant lifestyle context during chronic hepatitis B assessment."]),
        ("source.stom.liver.20260715", "Infoclinic", "STOM liver terminology and MRCM summary", "SNOMEDCT-20260701", "https://stom.infoclinic.co", "terminology_server", "2026-07-15", "not_due_existing_metadata_preserved", ["FHIR lookup confirmed active concepts for cirrhosis, liver steatosis, abnormal liver enzymes, jaundice, ascites, hepatic encephalopathy, chronic liver disease and portal hypertension.", "MRCM supports provisional semantic binding only and does not establish stage or urgency."]),
    ]
    artifacts = [{"id": i, "kind": "terminology_mrcm_query_summary" if profile == "terminology_server" else "clinical_guidance_metadata", "publisher": pub, "title": title, "version": version, "url": url, "language": "en", "digest": "live_response_summary_not_raw_cache" if profile == "terminology_server" else "metadata_only_not_cached", "license_status": "restricted", "complete": False, "monitor_profile": profile, "last_monitored_at": monitored, "monitor_result": result, "assertions": assertions} for i, pub, title, version, url, profile, monitored, result, assertions in defs]
...
def cases(f):
    ...
    handoff_required = set(policy["required_facts"]["always"] + policy["required_facts"]["routine"] + policy["conditional_required_facts"][0]["cases"]["alcohol_related_liver"] + policy["conditional_required_facts"][1]["cases"]["current"] + policy["conditional_required_facts"][2]["required_facts"])
    handoff = {}
    for fid in handoff_required:
        fact = by_id[fid]
        handoff[fid] = {"value": False if fact["value_type"] == "boolean" else fact.get("allowed_values", ["unclear"])[-1] if fact["value_type"] == "coded" else 0 if fact["value_type"] == "integer" else "없음"}
    handoff.update({
        "liver.primary_group": {"value": "alcohol_related_liver"},
        "patient.alcohol.use_status": {"value": "current"},
        "patient.alcohol.beverage_types": {"value": "soju"},
        "patient.alcohol.frequency": {"value": "주 3회"},
        "patient.alcohol.amount_per_occasion": {"value": "소주 4잔"},
        "liver.alcohol_heavy_episode_pattern": {"value": "월 1회 소주 1병 정도"},
        "liver.alcohol_last_use_timing": {"value": "어제 저녁"},
        "liver.alcohol_recent_reduction_or_cessation": {"value": True},
        "liver.alcohol_reduction_or_cessation_details": {"value": "2주 전부터 주 5회에서 주 3회로 감량"},
    })
    out["LIVER-ALCOHOL-ATOMIC-HANDOFF.json"] = {
        "id": "LIVER-ALCOHOL-ATOMIC-HANDOFF",
        "simulation_language": "ko",
        "clinician_submission": True,
        "persona": {"age": 52},
        "initial_statement": {"ko": "알코올 관련 간질환 추적 진료이고 현재 소주를 주 3회, 한 번에 4잔 정도 마십니다."},
        "hidden_state": handoff,
        "response_behavior": {"encounter.context_review_state": {"response_text": "최근 기본정보를 확인했고 바뀐 내용이 없습니다"}},
        "expected": {
            ...
            "expected_selected_facts_contains": ["patient.alcohol.use_status", "patient.alcohol.beverage_types", "patient.alcohol.frequency", "patient.alcohol.amount_per_occasion", "liver.alcohol_heavy_episode_pattern", "liver.alcohol_last_use_timing", "liver.alcohol_recent_reduction_or_cessation", "liver.alcohol_reduction_or_cessation_details"],
            "expected_selected_facts_excludes": ["liver.alcohol_pattern_and_last_use", "patient.alcohol.pattern"],
            "expected_known_facts": {"patient.alcohol.use_status": "current", "patient.alcohol.beverage_types": "soju", "patient.alcohol.frequency": "주 3회", "patient.alcohol.amount_per_occasion": "소주 4잔", "liver.alcohol_recent_reduction_or_cessation": True},
            "forbidden_assertions": ["diagnosis.alcohol_related_liver_disease", "recommendation.stop_alcohol_abruptly"],
        },
        "provenance": provenance(["source.nice.ng50.cirrhosis.2023", "specifications/clinical-memory.md"]),
    }
    return out
