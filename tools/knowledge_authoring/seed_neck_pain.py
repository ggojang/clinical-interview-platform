...
SOURCES = [
    ...
    "source.nhs.ekhuft.neck-aftercare.2026",
    "source.stom.neck-pain.20260716",
    "source.stom.neck-pain.20260805",
]
...
def fragment():
    e = [
        ...
        Q("neck.gait_or_balance_change", "New Gait or Balance Change", "boolean", "gait-balance-warning", "목 증상과 함께 새로 걷기가 불안해지거나 균형을 잡기 어려워졌나요?", 207, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.hand_clumsiness", "New Hand Clumsiness", "boolean", "hand-clumsiness-warning", "목 증상과 함께 새로 단추 잠그기·글씨 쓰기 같은 세밀한 손동작이 서툴거나 물건을 자주 떨어뜨리나요?", 206, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.progressive_limb_weakness", "Progressive Limb Weakness", "boolean", "progressive-weakness-warning", "목 증상과 함께 팔이나 다리의 힘 빠짐이 새로 생겼거나 빠르게 심해지고 있나요?", 205, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.new_bladder_function_change", "New Bladder Function Change", "boolean", "bladder-warning", "목 증상과 함께 소변이 갑자기 잘 나오지 않거나 참기 어려움·새는 증상이 새로 생겼나요?", 204, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.new_bowel_function_change", "New Bowel Function Change", "boolean", "bowel-warning", "목 증상과 함께 대변 조절이 어렵거나 본인도 모르게 새는 증상이 새로 생겼나요?", 203, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.new_saddle_sensory_change", "New Saddle Sensory Change", "boolean", "saddle-warning", "목 증상과 함께 엉덩이·회음부·성기 주변 감각이 새로 둔해졌나요?", 202, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        Q("neck.electric_shock_sensation", "Electric Shock Sensation", "boolean", "electric-shock-warning", "목을 따라 등·팔·다리로 번지는 전기충격 같은 느낌이 새로 생겼나요?", 201, [G["safety"], G["neurologic"]], S, safety_relevant=True),
        ...
        Q("neck.onset_date_time", "Onset Date and Time", "string", "onset-time", "목 증상이 처음 시작된 날짜와 대략적인 시각을 알려주세요.", 188, [G["character"]], C),
        Q("neck.onset_mode", "Onset Mode", "coded", "onset-mode", "목 증상은 갑자기 시작했나요, 서서히 시작했나요?", 187, [G["character"]], C, allowed_values=["sudden", "gradual", "unclear"]),
        Q("neck.duration_course_frequency_and_progression", "Course and Frequency", "string", "course", "계속되는지 간헐적인지, 한 번에 얼마나 지속되는지와 좋아짐·악화·반복 양상을 알려주세요.", 186, [G["character"]], C),
        ...
        Q("neck.current_pain_nrs", "Current Pain NRS", "integer", "current-pain-nrs", "현재 목 통증을 0점부터 10점까지 숫자로 답해 주세요. 0점은 통증 없음, 10점은 상상할 수 있는 가장 심한 통증입니다.", 185, [G["character"]], C, minimum=0, maximum=10, terminology_binding={"system": "http://loinc.org", "code": "72514-3", "display": "Pain severity - 0-10 verbal numeric rating [Score] - Reported", "version": "2.82", "relation": "equivalent"}),
        Q("neck.worst_pain_nrs", "Worst Pain NRS", "integer", "worst-pain-nrs", "이번 목 증상이 가장 심했을 때 통증을 0점부터 10점까지 숫자로 답해 주세요.", 184, [G["character"]], C, minimum=0, maximum=10),
        ...
        Q("neck.accessibility_or_communication_need", "Accessibility or Communication Need", "string", "accessibility", "문진이나 진료에 통역, 청각 지원, 큰 글씨·음성·쉬운 설명, 보호자 동석 등 필요한 지원이 있나요?", 134, [G["routing"], G["function"]], R),
    ...
    rules = [safety_rule(P, k, {"fact": f, "equals": True}, l, 1000 if l == "emergency" else 990) for k, f, l in safety]
    rules.extend([
        safety_rule(P, "cord-warning", {"any": [{"fact": fid, "equals": True} for fid in ["neck.gait_or_balance_change", "neck.hand_clumsiness", "neck.progressive_limb_weakness"]]}, "urgent", 990),
        safety_rule(P, "bladder-bowel-saddle-warning", {"any": [{"fact": fid, "equals": True} for fid in ["neck.new_bladder_function_change", "neck.new_bowel_function_change", "neck.new_saddle_sensory_change"]]}, "emergency", 1000),
        safety_rule(P, "electric-shock-warning", {"fact": "neck.electric_shock_sensation", "equals": True}, "urgent", 990),
    ])
    safety_refresh = {"class": "safety_critical", "last_assessed_at": "2026-08-05", "next_monitor_at": "2026-08-06", "next_full_review_at": "2026-11-03", "policy_id": "policy.knowledge-refresh"}
    for rule in rules:
        rule["refresh"] = safety_refresh
...
def source_docs():
    defs = [
        ("source.nice.ng127.cervical.2023", "NICE", "Suspected neurological conditions: recognition and referral", "NG127-updated-2023", "https://www.nice.org.uk/guidance/ng127/chapter/Recommendations-for-adults-aged-over-16", "clinical_guideline", "2026-08-05", ["Cervical radiculopathy referral assessment includes gait disturbance, clumsy or weak hands or legs and new bladder or bowel disturbance.", "The 2023 update remains the current published NICE version; runtime does not infer a cervical diagnosis."]),
        ("source.acr.cervical-pain-radiculopathy.2024", "American College of Radiology", "ACR Appropriateness Criteria: Cervical Pain or Cervical Radiculopathy", "revised-2024", "https://acsearch.acr.org/docs/69426/Narrative/", "clinical_guideline", "2026-08-05", ["The current revised-2024 document distinguishes acute or increasing pain, radiculopathy, trauma, infection, malignancy and prior cervical surgery contexts.", "The source supports collecting imaging-pathway context but is not used by runtime to order imaging or diagnose."]),
        ("source.cdc.meningococcal-symptoms.2026", "CDC", "Meningococcal Disease Symptoms and Complications", "2026-03-06", "https://www.cdc.gov/meningococcal/symptoms/", "public_health_guidance", "2026-08-05", ["Meningitis symptoms can include fever, headache, stiff neck, confusion, nausea or vomiting and photophobia.", "Meningococcal disease can worsen rapidly and requires immediate medical attention."]),
        ("source.nhs.ekhuft.neck-aftercare.2026", "East Kent Hospitals University NHS Foundation Trust", "Neck pain / injury: aftercare advice", "reviewed-2026-07", "https://leaflets.ekhuft.nhs.uk/neck-pain-injury-aftercare-advice/html/", "public_health_guidance", "2026-08-05", ["The July 2026 patient leaflet separates weakness, walking or balance difficulty, electric-shock sensation and bladder or bowel loss as urgent warning features.", "The leaflet explicitly requests accessibility, interpreter and alternative-format needs, supporting a distinct handoff Fact."]),
        ("source.stom.neck-pain.20260716", "Infoclinic", "STOM neck-pain terminology and MRCM lookup", "SNOMEDCT-20260701", "https://stom.infoclinic.co", "terminology_server", "2026-07-16", ["Build-time lookup confirmed active Neck pain (finding) 81680005, Cervical radiculopathy (disorder) 54404000 and Neck stiffness (finding) 161882006 with Korean descriptions.", "MRCM lookup for Neck pain returned Severity 246112005 and Finding site 363698007 among allowed attributes; terminology does not determine urgency or diagnosis."]),
        ("source.stom.neck-pain.20260805", "Infoclinic", "STOM LOINC verification for atomic current pain score", "LOINC-2.82", "http://localhost:8088/fhir", "terminology_server", "2026-08-05", ["FHIR CodeSystem lookup confirmed LOINC 72514-3 as Pain severity - 0-10 verbal numeric rating [Score] - Reported in version 2.82.", "Only the atomic current pain score uses this equivalent mapping; worst pain and composite pain questions do not inherit it."]),
    ]
    artifacts = [{"id": i, "kind": "terminology_mrcm_query_summary" if profile == "terminology_server" else "clinical_guidance_metadata", "publisher": pub, "title": title, "version": version, "url": url, "language": "en", "digest": "live_response_summary_not_raw_cache" if profile == "terminology_server" else "metadata_only_not_cached", "license_status": "restricted" if pub in {"American College of Radiology", "Infoclinic"} else "unknown", "complete": False, "monitor_profile": profile, "last_monitored_at": monitored, "monitor_result": "current_official_source_confirmed", "assertions": assertions} for i, pub, title, version, url, profile, monitored, assertions in defs]
...
def cases(f):
    ...
    def routine(branch):
        ...
        for fid in required:
            ...
            elif fact["value_type"] == "coded": value = fact.get("allowed_values", ["없음"])[0]
            elif fact["value_type"] in {"integer", "decimal"}: value = fact.get("minimum", 0)
            else: value = "없음"
            values[fid] = {"value": value}
        values["pain.frequency"] = {"value": "none"}
        values["neck.primary_group"] = {"value": branch}; return values
    axial = routine("axial_pain_or_stiffness"); axial["neck.current_pain_nrs"] = {"value": 3}; axial["neck.worst_pain_nrs"] = {"value": 6}; axial["neck.function_sleep_work_selfcare_driving_and_activity"] = {"value": "컴퓨터 업무 중 불편하지만 일상생활은 가능"}
    out["NECK-ROUTINE-AXIAL-CLINICIAN-HANDOFF.json"] = {"id": "NECK-ROUTINE-AXIAL-CLINICIAN-HANDOFF", "simulation_language": "ko", "persona": {"age": 44}, "initial_statement": {"ko": "며칠 전부터 오른쪽 목이 간헐적으로 아파요."}, "hidden_state": axial, "expected": {"expected_safety_level": "routine", "expected_stop_reason": "all_required_targets_resolved", "expected_max_turns": 70, "expected_known_facts": {"neck.current_pain_nrs": 3, "neck.worst_pain_nrs": 6}, "forbidden_assertions": ["diagnosis.cervical_spondylosis"]}, "provenance": provenance(SOURCES)}
    ...
    absent = routine("other_unclear"); absent.pop("neck.onset_date_time"); absent.pop("neck.information_source_proxy_and_reliability")
    out["NECK-AMBIGUOUS-DATA-ABSENT.json"] = {"id": "NECK-AMBIGUOUS-DATA-ABSENT", "simulation_language": "ko", "persona": {"age": 72}, "initial_statement": {"ko": "목이 좀 이상한데 언제부터인지는 모르겠어요."}, "hidden_state": absent, "response_behavior": {"neck.onset_date_time": {"dataAbsentReason": "asked-unknown"}, "neck.information_source_proxy_and_reliability": {"dataAbsentReason": "asked-unknown"}}, "expected": {"expected_data_absent_reasons": {"neck.onset_date_time": "asked-unknown", "neck.information_source_proxy_and_reliability": "asked-unknown"}, "expected_safety_level": "routine", "expected_stop_reason": "required_targets_addressed_with_absent_data", "expected_max_turns": 70, "forbidden_assertions": ["diagnosis.cervical_spine_disorder"]}, "provenance": provenance(SOURCES)}
    ...
    accessibility = routine("axial_pain_or_stiffness"); accessibility["neck.accessibility_or_communication_need"] = {"value": "난청이 있어 큰 글씨와 보호자 동석이 필요함"}
    out["NECK-ACCESSIBILITY-CLINICIAN-HANDOFF.json"] = {"id": "NECK-ACCESSIBILITY-CLINICIAN-HANDOFF", "simulation_language": "ko", "persona": {"age": 77, "hearing_accessibility": "hard_of_hearing"}, "encounter_context": {"care_setting": "primary_care", "encounter_type": "new_encounter", "interview_initiator": "patient", "interview_mode": "chat", "available_information": ["no_previous_records"], "time_constraint": "scheduled", "clinical_responsibility": "decision_support"}, "initial_statement": {"ko": "목이 아픈데 난청이 있어 보호자가 같이 봅니다."}, "hidden_state": accessibility, "expected": {"expected_safety_level": "routine", "expected_stop_reason": "all_required_targets_resolved", "expected_known_facts": {"neck.accessibility_or_communication_need": "난청이 있어 큰 글씨와 보호자 동석이 필요함"}, "expected_max_turns": 75, "forbidden_assertions": ["diagnosis.cervical_spondylosis"]}, "provenance": provenance(SOURCES)}
    return out
...
def main():
    ...
    mapping = {"id": M, "version": VERSION, "status": "research_only", "review_status": "unreviewed", "terminology": {"system": SN, "version": "http://snomed.info/sct/900000000000207008/version/20260701", "source": "STOM"}, "focus_concepts": [{"code": c, "display": d, "concept_active": True, "attribute_count_returned": 0} for c, d in concepts], "verified_attribute_ids": ["246112005", "363698007"], "verified_exact_questions": [{"fact_id": "neck.current_pain_nrs", "system": "http://loinc.org", "code": "72514-3", "display": "Pain severity - 0-10 verbal numeric rating [Score] - Reported", "version": "2.82", "relation": "equivalent"}], "validation": {"method": "build_time_live_fhir_lookup_mapping_search_and_mrcm_summary", "checked_at": ACQUIRED_AT, "raw_response_cached": False, "complete_mrcm_snapshot": False, "clinical_rule_authority": False, "result": "provisional_pass"}, "neck_semantics": {"diagnosis_inferred": False, "etiology_inferred": False, "laterality_postcoordinated_without_site_validation": False, "runtime_terminology_query_required": False, "composite_questions_inherit_atomic_code": False}, "provenance": provenance(["source.stom.neck-pain.20260716", "source.stom.neck-pain.20260805"])}
