...
def source_docs():
    ...
    artifacts = [{"id": i, "kind": "terminology_mrcm_query_summary" if profile == "terminology_server" else "clinical_guidance_metadata", "publisher": pub, "title": title, "version": version, "url": url, "language": "en", "digest": "live_response_summary_not_raw_cache" if profile == "terminology_server" else "metadata_only_not_cached", "license_status": "restricted", "complete": False, "monitor_profile": profile, "monitor_interval_days": days, "last_monitored_at": "2026-07-27" if i in {"source.gina.strategy.2026", "source.gold.report.2026"} else "2026-07-15", "next_monitor_at": "2026-07-28" if i in {"source.gina.strategy.2026", "source.gold.report.2026"} else ("2026-08-14" if days == 30 else ("2026-07-22" if days == 7 else "2026-07-16")), "monitor_result": "upstream_version_changed_and_reviewed" if i == "source.gina.strategy.2026" else "current_official_source_confirmed", "assertions": assertions} for i, pub, title, version, url, profile, days, assertions in defs]
