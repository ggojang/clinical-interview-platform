...
def fragment():
    e = [
        ...
        Q("resp_followup.current_new_or_severe_chest_pain", "Current New or Severe Chest Pain", "boolean", "chest-pain", "호흡 악화와 함께 새롭거나 심한 가슴통증이 있나요?", 174, [G["safety"]], S, safety_relevant=True, terminology_binding={"system": SN, "code": "29857009"}),
        Q("resp_followup.current_syncope", "Current Syncope", "boolean", "syncope", "호흡 악화와 함께 실신했나요?", 174, [G["safety"]], S, safety_relevant=True, terminology_binding={"system": SN, "code": "271594007"}),
        Q("resp_followup.current_new_sustained_irregular_palpitations", "Current New Sustained Irregular Palpitations", "boolean", "new-arrhythmia", "호흡 악화와 함께 새로 생긴 매우 불규칙한 두근거림이 지속되나요?", 174, [G["safety"]], S, safety_relevant=True, terminology_binding={"system": SN, "code": "80313002"}),
        ...
        Q("patient.smoking.status", "Smoking Status", "coded", "smoking-status", "담배·니코틴 제품 사용 상태를 선택해 주세요. 1 현재 사용, 2 과거 사용 후 중단, 3 평생 사용한 적 없음", 133, [G["risk"]], R, allowed_values=["current", "former", "never"], reuse_existing=True),
        Q("patient.smoking.product_types", "Tobacco and Nicotine Product Types", "coded_or_string", "smoking-products", "현재 또는 과거에 사용한 제품을 모두 알려주세요. 1 일반담배, 2 궐련형 전자담배, 3 액상형 전자담배, 4 시가·파이프, 5 무연담배, 6 기타", 132, [G["risk"]], R, allowed_values=["combustible_cigarette", "heated_tobacco", "electronic_cigarette", "cigar_or_pipe", "smokeless_tobacco", "other"], reuse_existing=True),
        Q("patient.smoking.cigarettes_per_day", "Combustible Cigarettes per Day", "quantity", "cigarettes-per-day", "일반담배를 피우거나 피웠다면 하루 평균 몇 개비였나요? 일반담배를 사용하지 않았다면 0개비라고 답해 주세요.", 131, [G["risk"]], R, reuse_existing=True, unit="{cigarette}/d", minimum=0),
        Q("patient.smoking.duration_years", "Total Tobacco or Nicotine Use Duration", "quantity", "smoking-duration", "담배·전자담배 등 니코틴 제품을 실제 사용한 총 기간은 몇 년인가요? 중단한 기간은 제외해 주세요.", 130, [G["risk"]], R, reuse_existing=True, unit="a", minimum=0),
        Q("patient.smoking.quit_timing", "Smoking Cessation Timing", "date_or_period", "smoking-quit-timing", "과거 사용자라면 마지막으로 사용한 때 또는 금연을 시작한 때는 언제인가요? 정확한 날짜를 모르면 대략적인 기간을 알려주세요.", 129, [G["risk"]], R, reuse_existing=True),
        Q("resp_followup.pack_years_reported", "Recorded Pack-years", "quantity", "pack-years", "의료기록이나 이전 진료에서 확인된 갑년(pack-years) 값이 있나요? 기록된 값을 모르면 모른다고 답해 주세요.", 128, [G["risk"], G["copd"]], R, unit="{pack-year}", minimum=0),
        Q("resp_followup.passive_smoke_exposure_status", "Passive Smoke Exposure Status", "boolean", "passive-smoke-status", "현재 또는 과거에 가정이나 직장에서 다른 사람의 담배 연기에 반복 노출된 적이 있나요?", 127, [G["risk"]], R),
        Q("resp_followup.passive_smoke_exposure_details", "Passive Smoke Exposure Details", "string", "passive-smoke-details", "간접흡연에 주로 노출된 장소와 대략적인 기간을 알려주세요.", 126, [G["risk"]], R),
        Q("resp_followup.occupational_exposure_and_time_away_pattern", "Occupational Respiratory Exposure", "string", "occupation", "직업과 분진·연기·화학물질 노출, 근무일과 휴일·휴가 때 증상 차이를 알려주세요.", 125, [G["risk"], G["asthma"]], D),
    ...
    rules = [
        ...
        safety_rule(P, "chest-pain", {"fact": "resp_followup.current_new_or_severe_chest_pain", "equals": True}, "emergency", 990),
        safety_rule(P, "syncope", {"fact": "resp_followup.current_syncope", "equals": True}, "emergency", 990),
        safety_rule(P, "new-arrhythmia", {"fact": "resp_followup.current_new_sustained_irregular_palpitations", "equals": True}, "emergency", 990),
...
def completion(f):
    ...
    policy["conditional_required_facts"] = [
        {"selector_fact": "resp_followup.condition_group", "cases": {"asthma": asthma, "copd": copd, "asthma_copd_overlap": asthma + copd, "uncertain": ["resp_followup.spirometry_date_and_results", "resp_followup.imaging_and_specialist_assessment"], "other_unclear": ["resp_followup.other_detail_or_patient_priority"]}},
        {"selector_fact": "patient.smoking.status", "cases": {
            "current": ["patient.smoking.product_types", "patient.smoking.cigarettes_per_day", "patient.smoking.duration_years", "resp_followup.pack_years_reported"],
            "former": ["patient.smoking.product_types", "patient.smoking.cigarettes_per_day", "patient.smoking.duration_years", "patient.smoking.quit_timing", "resp_followup.pack_years_reported"],
            "never": []
        }},
        {"when": {"fact": "resp_followup.passive_smoke_exposure_status", "equals": True}, "required_facts": ["resp_followup.passive_smoke_exposure_details"], "reason": "reported_passive_smoke_exposure_requires_context"},
    ]
    return policy
...
def source_docs():
    defs = [
        ("source.nice.ng245.asthma.2025", "NICE", "Asthma: diagnosis, monitoring and chronic asthma management", "NG245; amended-2025-11", "https://www.nice.org.uk/guidance/ng245/chapter/recommendations", "nice_guidance", 7, ["Monitor symptoms, work or school absence, reliever use, oral corticosteroid courses and emergency or hospital attendance at every asthma review.", "Check adherence and inhaler technique at every asthma-related review; distinguish active or passive smoking and vaping exposure alongside occupational and environmental factors before treatment adjustment."]),
        ...
        ("source.gina.strategy.2026", "GINA", "Global Strategy for Asthma Management and Prevention", "2026-05", "https://ginasthma.org/2026-gina-strategy-report/", "clinical_guideline", 1, ["Assess asthma symptom control over the previous four weeks and future risk, including lung function, exacerbations and treatment adverse effects.", "At follow-up, observe inhaler technique, assess adherence, verify a written action plan, and ask about treatment attitudes, concerns and goals."]),
        ("source.gold.report.2026", "GOLD", "Global Strategy for Prevention, Diagnosis and Management of COPD", "2026", "https://goldcopd.org/2026-gold-report-and-pocket-guide/", "clinical_guideline", 1, ["COPD follow-up reassesses dyspnea and exacerbations and regularly checks inhaler technique, adherence, smoking and continued risk exposure; non-pharmacological treatment and action plans are reviewed.", "The 2026 change summary distinguishes initial from follow-up treatment and identifies one moderate or severe exacerbation as clinically important for subsequent-event risk assessment."]),
    ...
    artifacts = [{"id": i, "kind": "terminology_mrcm_query_summary" if profile == "terminology_server" else "clinical_guidance_metadata", "publisher": pub, "title": title, "version": version, "url": url, "language": "en", "digest": "live_response_summary_not_raw_cache" if profile == "terminology_server" else "metadata_only_not_cached", "license_status": "restricted", "complete": False, "monitor_profile": profile, "monitor_interval_days": days, "last_monitored_at": "2026-07-29" if profile != "terminology_server" else "2026-07-15", "next_monitor_at": "2026-08-14" if profile == "terminology_server" else "2026-07-30", "monitor_result": "not_due_existing_metadata_preserved" if profile == "terminology_server" else "current_official_source_confirmed_no_version_change", "assertions": assertions} for i, pub, title, version, url, profile, days, assertions in defs]
...
def cases(f):
    ...
    hidden.pop(declined)
    out["RESP-FOLLOWUP-ASTHMA-DATA-ABSENT.json"] = {"id": "RESP-FOLLOWUP-ASTHMA-DATA-ABSENT", "simulation_language": "ko", "persona": {"age": 42}, "initial_statement": {"ko": "천식 정기 진료를 받으러 왔고 요즘은 대체로 괜찮아요."}, "hidden_state": hidden, "response_behavior": {declined: {"dataAbsentReason": "asked-unknown"}}, "expected": {"expected_data_absent_reasons": {declined: "asked-unknown"}, "expected_safety_level": "routine", "expected_stop_reason": "required_targets_addressed_with_absent_data", "expected_max_turns": 65, "forbidden_assertions": ["diagnosis.controlled_asthma", "recommendation.step_down_treatment"]}, "provenance": provenance(["source.gina.strategy.2026", "specifications/clinical-memory.md"])}

    copd_required = set(policy["required_facts"]["always"] + policy["required_facts"]["routine"] + policy["conditional_required_facts"][0]["cases"]["copd"])
    copd_hidden = {}
    for fid in copd_required:
        fact = by_id[fid]
        if fact["value_type"] == "boolean": copd_hidden[fid] = {"value": False}
        elif fact["value_type"] == "coded": copd_hidden[fid] = {"value": fact.get("allowed_values", ["unclear"])[-1]}
        elif fact["value_type"] == "integer": copd_hidden[fid] = {"value": 0}
        else: copd_hidden[fid] = {"value": "없음"}
    copd_hidden.update({
        "resp_followup.condition_group": {"value": "copd"},
        "resp_followup.current_acute_change": {"value": "stable_review"},
        "resp_followup.exacerbations_last_12_months": {"value": 1},
        "resp_followup.oral_steroid_courses_last_12_months": {"value": 1},
        "resp_followup.antibiotic_courses_for_exacerbation": {"value": 1},
        "resp_followup.last_exacerbation_treatment_and_recovery": {"value": "6개월 전 외래에서 전신 스테로이드와 항생제를 사용했고 입원 없이 평소 상태로 회복"},
    })
    out["RESP-FOLLOWUP-COPD-MODERATE-EXACERBATION-HANDOFF.json"] = {"id": "RESP-FOLLOWUP-COPD-MODERATE-EXACERBATION-HANDOFF", "simulation_language": "ko", "clinician_submission": True, "persona": {"age": 71}, "initial_statement": {"ko": "COPD 정기 진료로 왔고 지난 1년 중 한 번 외래 치료를 받은 악화가 있었어요."}, "hidden_state": copd_hidden, "expected": {"expected_safety_level": "routine", "expected_stop_reason": "required_targets_addressed_with_absent_data", "expected_max_turns": 80, "expected_clinician_handoff": True, "expected_known_facts": {"resp_followup.exacerbations_last_12_months": 1, "resp_followup.oral_steroid_courses_last_12_months": 1, "resp_followup.antibiotic_courses_for_exacerbation": 1, "resp_followup.last_exacerbation_treatment_and_recovery": "6개월 전 외래에서 전신 스테로이드와 항생제를 사용했고 입원 없이 평소 상태로 회복"}, "forbidden_assertions": ["diagnosis.copd_exacerbation", "recommendation.escalate_inhaler_treatment"]}, "provenance": provenance(["source.gold.report.2026"])}
    smoking_hidden = dict(copd_hidden)
    smoking_hidden.update({
        "patient.smoking.status": {"value": "current"},
        "patient.smoking.product_types": {"value": "combustible_cigarette"},
        "patient.smoking.cigarettes_per_day": {"value": {"amount": 10, "unit": "{cigarette}/d"}},
        "patient.smoking.duration_years": {"value": {"amount": 20, "unit": "a"}},
        "resp_followup.pack_years_reported": {"value": {"amount": 10, "unit": "{pack-year}"}},
        "resp_followup.passive_smoke_exposure_status": {"value": False},
    })
    out["RESP-FOLLOWUP-ATOMIC-SMOKING-HANDOFF.json"] = {"id": "RESP-FOLLOWUP-ATOMIC-SMOKING-HANDOFF", "simulation_language": "ko", "clinician_submission": True, "persona": {"age": 63}, "initial_statement": {"ko": "COPD 정기 진료이고 현재 일반담배를 하루 10개비 정도 피웁니다."}, "hidden_state": smoking_hidden, "response_behavior": {"patient.smoking.cigarettes_per_day": {"response_text": "하루 10개비"}, "patient.smoking.duration_years": {"response_text": "20년"}, "resp_followup.pack_years_reported": {"response_text": "10"}}, "expected": {"expected_safety_level": "routine", "expected_stop_reason": "required_targets_addressed_with_absent_data", "expected_max_turns": 85, "expected_clinician_handoff": True, "expected_selected_facts_contains": ["patient.smoking.status", "patient.smoking.product_types", "patient.smoking.cigarettes_per_day", "patient.smoking.duration_years", "resp_followup.pack_years_reported", "resp_followup.passive_smoke_exposure_status"], "expected_selected_facts_excludes": ["resp_followup.smoking_vaping_and_pack_years", "resp_followup.passive_smoke_exposure_details"], "expected_known_facts": {"patient.smoking.status": "current", "patient.smoking.product_types": "combustible_cigarette", "patient.smoking.cigarettes_per_day": {"amount": 10, "unit": "{cigarette}/d"}, "patient.smoking.duration_years": {"amount": 20, "unit": "a"}, "resp_followup.pack_years_reported": {"amount": 10, "unit": "{pack-year}"}, "resp_followup.passive_smoke_exposure_status": False}, "forbidden_assertions": ["diagnosis.smoking_related_disease", "recommendation.change_inhaler_treatment"]}, "provenance": provenance(["source.nice.ng245.asthma.2025", "source.nice.ng115.copd.2025"])}
    return out
