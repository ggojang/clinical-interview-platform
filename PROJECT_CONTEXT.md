...
The next expansion queue, `knowledge/catalog/planned-package-work-queue-v0.3.json`, is active. Its materialized slices are grouped eye symptoms and grouped ear or hearing symptoms. Eye coverage includes red eye or ocular-surface discomfort, vision change, eyelid or periorbital symptoms, and trauma or foreign body. Ear and hearing coverage includes ear pain or infection features, hearing change, discharge or trauma, and tinnitus, with explicit branches for sudden hearing loss, neurological abnormalities, mastoid warning features, severe infection, dangerous foreign bodies and head injury. Both packages use conditional branch completion and build-time STOM terminology mapping; laterality is applied only when the selected finding site is verified in `723264001 |Lateralizable body structure reference set|`. All content remains `unreviewed/research_only`; MRCM and reference-set results constrain terminology representation only and never create clinical safety rules.
...
- Simulation and Evaluation cover only the twenty-three currently compiled vertical slices and not the full Primary Care scope;
