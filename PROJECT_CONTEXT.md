...
The current implementation is an early multi-RFE executable Knowledge Factory with cough, fever, and breathing-difficulty vertical slices.

It includes a profile-driven deterministic Builder, a Primary Care Reason for Encounter catalog, 62 unique research Facts across independently compiled cough, fever, and breathing-difficulty packages, complete Fact-to-Question linkage within all three packages, shared Fact identity, versioned Source Manifests, source-specific refresh scheduling, deterministic Compiler, integrity-checked draft Knowledge Packages, evidence-preserving Clinical Memory with coded `dataAbsentReason`, package-driven multi-turn Runtime, respiratory and systemic warning-feature examples, limited English and Korean extraction, thirty-three synthetic JSON Simulations, Evaluation, computed Coverage, validation and tests.
...
- Compiler and Knowledge Packages currently cover only cough, adult fever, and breathing-difficulty vertical slices;
