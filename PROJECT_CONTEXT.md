...
Every dynamic interview question displays response guidance. Numbered choices are shortcuts rather than a closed answer set, so the user is told that an unlisted answer may be entered directly; open questions explicitly invite free text. Required FHIR-bound values may trigger a targeted clarification before standard storage, and source-defined fixed questionnaires retain their official response constraints.
...
Smoking and alcohol history are collected as conditional atomic Facts rather than one composite completion flag. Current or former tobacco use activates separate product-type, combustible-cigarette amount, total-duration and, when applicable, cessation-timing questions. Current or former alcohol use activates separate beverage-type, frequency and amount-per-occasion questions. Explicit details volunteered in a richer symptom-specific answer prefill the matching atomic Facts, so only genuinely missing dimensions are asked later.
...
`rfe.immunization_consultation` provides the dedicated research-only pre-visit workflow for planned vaccination, vaccination-record review, catch-up review, travel or occupational exposure review, and post-vaccination concerns. It collects record provenance and conflicts, vaccine and dose timing, vaccine-specific precaution history, pregnancy and immune context, recent vaccines and antibody-containing blood products, travel or exposure context, current post-vaccination warning symptoms, and the person's goal. Its compiled boundary explicitly forbids inferring whether a vaccine is due, eligible, recommended or contraindicated; those conclusions require a current Korean schedule, age/risk/jurisdiction context and clinician review. KDCA guidance supplies the clinical source boundary, while FHIR R4/KR Core structures and build-time STOM lookups support representation only.

`rfe.preoperative_assessment` provides a dedicated research-only pre-visit workflow for planned surgery assessment, preparation-instruction review, prior-test handoff, reassessment after delay or cancellation, and newly changed health before surgery. It collects the planned procedure and source reliability, functional capacity, current time-sensitive symptoms, cardiopulmonary and major comorbidity context, bleeding and thrombosis history, current medicines and received instructions, allergies, previous anaesthesia problems, pregnancy possibility, existing tests, access needs and the person's questions. The package never grants surgical or anaesthetic clearance, decides whether surgery should proceed, calculates a risk score, orders tests or tells a person to start, stop or bridge a medicine. Those decisions require the responsible surgical and anaesthesia teams, local Korean protocols and clinician review.
...
For an RFE with an audited package-specific clinician minimum, those detailed Facts satisfy the generic current-problem domains and the four fallback questions for prior evaluation, attempted treatment, functional impact and patient goals are not asked again. The generic questions remain available only when an RFE lacks an audited specific minimum.
...
Reference ValueSets are checked by canonical, version and normalized membership
before creation. Identical existing content with the same canonical/version is
reused; membership-equivalent content under a different canonical is only
reported and never substituted. If the requested canonical/version is absent,
the reference is created without changing its contents. Any
application-specific extension is a distinct `a-extended-*` ValueSet that
imports the untouched versioned reference and adds only independently verified
codes. Dynamic extensible questions may use FHIR R4 `open-choice`; fixed
instruments and required closed answer sets are not expanded.
...
The current implementation is an early multi-RFE executable Knowledge Factory with fifty-six independently compiled Primary Care Knowledge Package profiles. Every dynamic question now has a complete local CodeSystem identity, verified LOINC and secondary SNOMED CT mappings are compiled where available, and coded answer choices use verified SNOMED CT or a context-qualified local fallback. The complete local question and answer CodeSystems are registered on the Build-Time STOM server, with exact canonical/version/content reconciliation and direct code validation required before dependent ValueSet publication. Complete answer ValueSets are generated for SNOMED-only, mixed SNOMED/local and local-only choices, and a repository-wide atomicity gate prevents composite candidates from receiving exact or equivalent standard question mappings. Repository-wide mapping Coverage is computed rather than inferred. The gait and falls profile distinguishes an acute fall, recurrent falls, gait or balance change, fear or near-falls, post-injury follow-up and known mobility-condition follow-up. The epistaxis profile records current bleeding, pressure response, observable amount, duration, frequency, laterality, trauma, antithrombotic and bleeding history. The paediatric growth and development profile preserves dated raw growth measurements and their source, corrected-age context, skills never acquired versus lost, development across domains and settings, feeding, sensory, perinatal, family, education, social, standardized-screen provenance, intervention and family goals. It does not calculate centiles, reproduce licensed screening instruments or infer a diagnosis. The tremor and movement-concern profile records body distribution and side, rest-posture-action-task relationship, rhythm and modifiability, slowness, stiffness, gait, other involuntary movements, bulbar and non-motor features, medicines, stimulants, alcohol, metabolic and toxin context, prior evaluation, function and hazardous-activity safety. It preserves movement classification and diagnostic uncertainty, does not interpret patient video, and does not recommend medication changes.

The dedicated test-result follow-up profile separates an institution result-check visit from a request for interpretation or both. An institution-only visit remains minimal and does not ask the patient to upload a result; an interpretation request may ask once only when the content is not already available. The handoff preserves the exact test or report identity, clinically relevant and issued times, report status and amendment, performing organisation, information source and readability, reported values, units, reference ranges and report conclusion without silently interpreting them. It records prior professional explanation, known follow-up plan, conflicts, absent-data reason, the patient's question and expected help. Escalation is limited to an explicit emergency or urgent instruction and separately captured current severe or rapidly worsening symptoms. It does not infer a diagnosis, classify a result as clinically normal or abnormal, or replace the issuing or treating clinician's interpretation.

The test-result follow-up profile also preserves why the test was requested, who requested and interpreted it, specimen and collection context, body site, method, device, report limitations, pending components and expected completion. It keeps any prior result and comparison exactly as reported rather than inferring a trend. For closed-loop handoff it records when and how the patient was notified, who is responsible for the next step, the recommended action and due time, action status, barriers and communication support needs. These research-only additions are grounded in HL7 FHIR R4 Observation and DiagnosticReport semantics, AHRQ diagnostic-safety follow-up guidance, the Australian Commission pathology reporting requirements and read-only STOM terminology checks.

The dedicated post-discharge follow-up profile prepares a transition-of-care handoff without inferring a complication or readmission need. It preserves the discharge date and institution, information source and document reliability, reported admission reason and major treatment when the discharge record is incomplete, current course and new symptoms, the actual medicine list and changes, discrepancies or access problems, pending results and their owner, follow-up appointments and access barriers, home services, caregiver availability, contact instructions, plan understanding and the person's concerns. Following AHRQ RED, CMS discharge-planning and NICE transition-of-care guidance, it now separates wound-care tasks from home medical devices or equipment and records instructions, training, receipt, use difficulty and a known help contact as atomic handoff facts. It also separates receipt of emergency-return instructions from the presence of a currently instructed warning feature, and routes only explicitly captured time-sensitive changes. LOINC 11544-4 is retained as a discharge-follow-up record-title reference rather than misrepresented as an exact question code; SNOMED CT 308283009 is retained as discharge-event context. All questions remain local and unreviewed until exact question semantics and answer bindings are clinically and terminologically reviewed.

The dedicated tobacco and nicotine counselling profile prepares a product-specific pre-visit handoff rather than reducing social history to a single smoking-status answer. It conditionally separates combustible cigarettes, heated tobacco, electronic cigarettes, cigars or pipes and smokeless tobacco; records amount, duration, last use, nicotine content where relevant, time to first use after waking, craving or withdrawal, prior quit attempts and support response, readiness and target timing, secondhand exposure, pregnancy or postpartum context, relevant conditions, medicines, allergies and the person's concern and expected help. Acute severe chest pain, severe breathing difficulty, collapse or reduced consciousness and suspected acute nicotine-product exposure are routed without diagnosing a tobacco-related condition or selecting cessation treatment. LOINC 72166-2, 67741-9 and 105045-9, official LOINC answer lists LL2201-3 and LL6587-1, and active SNOMED CT smoking-status concepts were verified through STOM; all mappings remain unreviewed and research-only.

The dedicated alcohol-use counselling profile prepares a raw, culturally interpretable pre-visit handoff rather than treating current, former or never use as a complete alcohol history. It preserves beverage types, frequency, type-specific amount per occasion, largest-day amount, last use, total duration and usual setting; records recent reduction, craving, control difficulty, morning use, tolerance, prior withdrawal seizure, delirium or medically assisted withdrawal, previous change attempts and support response, readiness, target timing, injury or hazardous-use context, role, physical, mental-health and sleep impact, co-used sedating substances, pregnancy or postpartum context, relevant conditions, medicines, allergies, support and patient goals. Reduced consciousness, current seizure, severe confusion or hallucination, intentional or mixed overdose, immediate self or other harm, and withdrawal warning features after recent reduction are routed without diagnosing intoxication, alcohol use disorder or withdrawal. KDCA, NICE CG115 and CG100, the NIAAA Core Resource and STOM-verified LOINC 68518-0, 74014-2 and related 11287-0 support the research package. AUDIT, AUDIT-C and CIWA-Ar remain separately governed instrument references and are not reproduced, scored or automatically mapped as dynamic questions.
...
The dedicated physical-activity counselling profile prepares a measurable pre-visit handoff rather than reducing activity to active or inactive. It preserves activity type and life domain, recent seven-day activity days and minutes, muscle-strengthening and balance frequency, sedentary time and interruption pattern, current-pattern duration, change from baseline and information source. It separately records current emergency symptoms, exertional syncope, chest discomfort, breathlessness, dizziness, palpitations, pain, fatigue, falls concern and daily-function limits; mobility aids, cardiopulmonary, musculoskeletal, neurologic and metabolic context; pregnancy or postpartum status, accessibility needs, medicines, prior restrictions, injury or rehabilitation, environment, occupation, previous attempts, barriers, support, readiness and patient goals. It does not calculate fitness, MET-minutes or guideline adherence and does not prescribe exercise. KDCA, WHO and HHS official guidance and STOM-verified LOINC 68515-6, 68516-4 and 82291-6 support the research package; the LOINC Exercise Vital Sign panel remains a verified panel reference rather than a replacement for atomic items.

The dedicated swallowing-difficulty profile separates a new swallowing Reason for Encounter from the dyspepsia and reflux route. It preserves the person's wording, information source and reliability; exact onset, latest episode, course and frequency; difficulty initiating a swallow and perceived sticking location; separate solid-food, soft-food and liquid difficulty; painful swallowing and conditional raw 0–10 pain NRS; chewing, oral residue, nasal return, cough, choking, wet voice, breathing change, regurgitation, reflux and meal-related chest context; meal duration, fatigue, avoidance, intake, weight, recurrent chest infection, daily function and social impact; neurological, head-and-neck, oesophageal, oral, respiratory, cognitive, posture, device, medicine, allergy, prior-assessment, assistance, accessibility and patient-goal context. Research-only Safety Rules route complete airway obstruction, reduced consciousness, blue colour, sudden swallowing difficulty with focal neurological change, inability to swallow saliva, suspected food bolus with inability to swallow liquids, severe breathlessness after swallowing and poor fluid intake with markedly reduced urine without diagnosing aspiration, stroke, obstruction, malignancy or another cause. NHS, NICE, ASHA and RCSLT official material supports the clinical dimensions. STOM verifies the dysphagia focus concept and LOINC 72514-3 for pain NRS; official FACIT LOINC swallowing items 70367-8, 70368-6 and 70369-4 are retained only as related metadata because their source-defined answer semantics are not adopted. The observed SNOMED CT 20260801 service version is recorded as a terminology-baseline review trigger and does not silently replace the repository 20260701 baseline.

The dedicated jaundice profile separates a newly observed yellow-eye or yellow-skin presentation from chronic liver-function follow-up. It preserves the person's words, information source and reliability; who noticed the change; exact onset, latest observation, course and prior episodes; separate scleral and skin observations with lighting and visibility uncertainty; dark urine, pale stool and itching; abdominal symptoms, temperature, vomiting, appetite, weight, fatigue, swelling, bruising, sleep and function impact; liver, biliary, pancreatic, blood, surgery, medicine, supplement, alcohol, acetaminophen, blood or body-fluid, travel, food or water, contact, infection, allergy and pregnancy context; and previous assessment, atomic bilirubin results and dates, imaging, previous response, accessibility and patient goals. Research-only Safety Rules route reduced consciousness, new confusion, collapse, severe right-upper abdominal pain with fever or rigors, repeated vomiting with inability to retain fluids and markedly reduced urine, gastrointestinal or unexplained bleeding, suspected overdose or toxic exposure, newborn jaundice, an unwell newborn, pregnancy or early postpartum status and rapidly worsening yellowing without diagnosing hepatitis, obstruction, infection, malignancy, liver failure or another cause. KDCA, NHS, CDC and NICE official material supports the clinical dimensions; UK referral detail is retained as source context and is not projected into Korean care policy. STOM verifies SNOMED CT 18165001 for RFE indexing only and LOINC 1975-2, 1968-7 and 72514-3 for atomic total-bilirubin, direct-bilirubin and pain-NRS questions. The observed SNOMED CT 20260801 service version remains a baseline-review trigger rather than a silent global upgrade.

The dedicated acute-confusion profile separates a sudden mental-status or behaviour change from chronic memory and cognitive concern. It preserves the original wording, information source and reliability, a collateral baseline informant, exact onset and last-known-usual time, course and fluctuation, attention, orientation, thought or speech organisation, perception, activity, sleep and usual cognitive-functional state. Its time-sensitive gate checks reduced consciousness, a confirmed acute change, focal neurological change, seizure, recent head injury, severe breathing difficulty or chest pain, toxic or carbon-monoxide exposure, a glucose-device low alert, immediate harm danger, severe illness and absence of safe supervision before routine context. The professional handoff then preserves temperature, infection, pain, hydration, nutrition, elimination, falls, recent hospital or environmental change, prior cognitive impairment or delirium, medicines, alcohol and other substances, sensory aids, pregnancy context, previous assessment and tests, function, safety, accessibility and patient or caregiver goals. It does not diagnose delirium, dementia, stroke, infection, intoxication or another cause; administer CAM, 4AT or another source-defined instrument; or recommend tests or treatment. NHS, NICE and the Australian Delirium Clinical Care Standard support the research dimensions. STOM verifies SNOMED CT 2776000 and 62476001 as indexing or related references and LOINC 72514-3 for the atomic pain-NRS question. CAM/CMS LOINC items are retained only as related source-defined references and are not emitted as equivalent questions.

The strengthened neck-pain profile separates hand clumsiness, gait or balance change, progressive limb weakness, new bladder change, new bowel change, saddle sensory change and an electric-shock sensation into atomic Facts. Grouped Safety Rules evaluate those independent answers without requiring a compound patient response and without inferring cervical myelopathy, radiculopathy or another diagnosis. The handoff now preserves separate onset time and onset mode, current and worst 0–10 pain scores, shared pain frequency and accessibility or communication support needs. The atomic current pain score uses STOM-verified LOINC 72514-3; the worst pain score remains local because no equivalent code was verified. NICE NG127 and the July 2026 East Kent Hospitals NHS neck-pain aftercare leaflet support the neurological warning and accessibility dimensions. All additions remain unreviewed and research-only.

The strengthened epistaxis profile no longer asks one compound haemodynamic-warning question. It records syncope, dizziness or lightheadedness, clouded consciousness, chest pain and dyspnea as independent boolean Facts, while blood obstructing the airway or inability to handle blood remains a separate emergency Fact. Independent Safety Rules preserve each finding in clinician handoff and stop routine collection at the applicable emergency or urgent boundary without diagnosing shock, anaemia, posterior epistaxis or another cause. The NSW Agency for Clinical Innovation emergency-care fact sheet and the current Royal Children's Hospital guideline support these atomic warning dimensions. Build-time STOM lookup verifies the corresponding SNOMED CT findings against the 20260801 service version; this targeted mapping does not allow terminology to determine urgency and does not silently replace unrelated package mappings. All additions remain unreviewed and research-only.

The strengthened skin profile now treats hair loss and scalp change as an explicit branch rather than forcing those presentations through a generic rash or lesion path. It first distinguishes diffuse shedding, gradual thinning, a localised patch, hairline or breakage change, crown-centred change and an unclear pattern, then conditionally asks only the trigger, scalp-observation, hair-practice, contact and prior-care Facts relevant to that pattern. The handoff preserves onset and course, exact site, active shedding, scale, redness, itch, pain NRS, burning, tenderness, smooth shiny change, broken hairs, non-scalp hair loss, recent illness, fever, surgery, childbirth, weight loss, restrictive diet, stress, medicine or cancer-treatment context, iron and thyroid history, family history, tension, heat and chemical exposure, household or animal contact, prior evaluation and treatment response, emotional impact, source reliability and absent-data reason. A hot, painful and swollen skin or scalp cluster routes to urgent human handoff without diagnosing infection, and possible scarring, fungal, autoimmune, hereditary or temporary shedding causes are never assigned by Runtime. NHS, British Association of Dermatologists, NHS Scotland and Nottinghamshire APC material supports the research dimensions. Build-time STOM lookup verifies Loss of hair 278040002, Scarring alopecia 400088006 and Scalp structure 41695006 against the 20260801 service version and confirms Finding site and Severity MRCM availability for the two hair-loss focus concepts. All additions remain unreviewed draft content.

The strengthened ear and hearing profile records pulse-synchronous tinnitus, tinnitus-associated hearing change and tinnitus-associated vertigo as independent Facts rather than compound answers. A pulse-synchronous answer routes to urgent human handoff, and tinnitus with associated vertigo routes to emergency human handoff, without inferring a vascular, neurological or vestibular diagnosis. The pulse-synchronous question is promoted into the safety-gate budget so remote, caregiver-assisted and accessibility-constrained interviews cannot omit it before an 18-question limit. NHS tinnitus guidance and NICE NG155 support the referral dimensions; STOM verifies Subjective pulsatile tinnitus 117421000119104 against the 20260801 SNOMED CT service version. These additions remain unreviewed draft content with limited-use status.

Generated clinical content now declares lifecycle status `draft`, review status `unreviewed`, and clinical-use status `limited`. The existing `research_only` value is retained only as a legacy schema and transport compatibility label until a versioned migration is complete. Draft packages may support research testing, simulation, clinician-supervised pilots, pre-visit information collection and general informational responses, but they have no independent diagnosis, treatment-selection, prescribing or medication-change authority. When diagnosis or treatment is requested, the service may provide general information with uncertainty, red flags and clinician discussion topics only. Safety Rules have protective handoff authority rather than diagnostic authority: a suspected red flag is surfaced promptly, with sensitivity favoured even at the cost of false positives, and routine interview completion never delays the warning.

An additive service-mode overlay now exposes adaptive clinical preparation, supplied clinical Questionnaires, verified fixed conversational surveys, supplied non-clinical surveys, screening-center add-on comparison and general health information. No explicit selection still routes to the existing Reason-for-Encounter-first Chatbot, and its four conversation starters and read-only public Knowledge Action remain unchanged. Fixed or supplied Questionnaires retain source authority. Screening recommendation starts with the supplemental adaptive interview; the official NHIS questionnaire is optional and remains a separate Questionnaire/QuestionnaireResponse pair. The reusable FHIR R4 prepopulation primitive applies only versioned exact/equivalent mappings, requires all atomic source Facts for compound items and leaves the response `in-progress` for user review. The official NHIS Questionnaire and verified linkId map remain a declared gap rather than an invented resource. Current-process clinical and screening sessions now expose explicit purge operations for response-bearing state; external ChatGPT retention remains outside repository control.

The adaptive clinical service now uses the Custom GPT test interaction contract as its patient-facing default. Compiled authoring questions remain available for audit and handoff, but the UI receives one concise answer-bearing stem, audited answer shortcuts or non-coded examples, and an 18-question ordinary-visit soft budget. An optional bounded current-turn interpreter may map one answer to several allowlisted Facts; Runtime validates identifiers, types, allowed tokens, units and ranges before merge. Historical answer values are not sent for this extraction. Compiled Safety Rules remain authoritative, and every compiled Fact not asked within the concise path remains explicitly `not-asked` in the handoff rather than becoming a negative answer.
...
- Compiler and Knowledge Packages currently cover fifty-six materialized Primary Care profiles and not the full Primary Care scope;
...
- Simulation and Evaluation cover only the fifty-six currently compiled profiles and not the full Primary Care scope;
