...
The current implementation is an early multi-RFE executable Knowledge Factory with cough, fever, breathing-difficulty, abdominal-pain, chest-pain, headache, dizziness-or-syncope and vomiting-or-diarrhoea vertical slices.

It includes a profile-driven deterministic Builder, a Primary Care Reason for Encounter catalog, 173 unique research Facts across eight independently compiled packages, complete Fact-to-Question linkage within every package, shared Fact identity, versioned Source Manifests, source-specific refresh scheduling, deterministic Compiler, integrity-checked draft Knowledge Packages, evidence-preserving Clinical Memory with coded `dataAbsentReason`, package-driven multi-turn Runtime, multi-domain warning-feature examples, limited English and Korean extraction, 96 synthetic JSON Simulations, Evaluation, computed Coverage, validation and tests.
...
- Compiler and Knowledge Packages currently cover only cough, adult fever, breathing difficulty, abdominal pain, chest pain, headache, dizziness-or-syncope and vomiting-or-diarrhoea vertical slices;
...
- Simulation and Evaluation cover only the eight currently compiled vertical slices and not the full Primary Care scope;
