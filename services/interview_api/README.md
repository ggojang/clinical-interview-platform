...
자유 진료 전 문진은 명시적 완료 뒤 실제 질문 이력을 FHIR R4 `Questionnaire`와 `QuestionnaireResponse`로 투영하고, 컴파일된 Fact 종류에 따라 `Patient`, `Encounter`, `Condition`, `Procedure`, `MedicationStatement`, `AllergyIntolerance`, `FamilyMemberHistory`, `Observation` 후보를 담은 draft SDC Extraction `Bundle`을 생성합니다. 질문은 선언된 LOINC item code가 있을 때 이를 사용하고 그 밖에는 등록된 local Question CodeSystem을 사용합니다. boolean 답변은 검증된 SNOMED CT yes/no를 사용하며, 표준 매핑이 없는 자유문은 local Fact code와 원문을 보존합니다. 이 산출물은 `draft_projection`, `unreviewed`, clinician review required이며 공식 SDC profile 적합성이나 진단을 주장하지 않습니다.

완료 전 데모 UI의 Questionnaire/QuestionnaireResponse는 공통 요소를 이용한 **브라우저 draft**이며 최종 산출물로 표시하지 않습니다. 완료 뒤에는 서버가 생성한 R4 Questionnaire/QuestionnaireResponse와 draft extraction을 표시합니다. 공식 FHIR validator 및 전체 SDC profile 적합성은 별도 배포 게이트입니다. 이미지 설문은 브라우저 미리보기만 지원하고 OCR·문항 구조화는 아직 지원하지 않습니다. 건강검진 패키지 추천은 별도 임상 설문이나 Fact를 만들지 않고 컴파일된 국가건강검진 문진과 공통 clinician context의 기존 Fact·질문을 답변 맥락에 따라 재사용합니다. 같은 나이·성별·질환·약물·가족력·흡연·음주 Fact가 문진, 검진 대상 판단, 패키지 비교에 다시 쓰이며, 지역·가격·추천 후 국가검진 작성 여부만 비교 작업의 운영 질문으로 분리됩니다. 세션은 `/v1/sessions/{session_id}/attachments`에서 PDF, JSON/FHIR, 텍스트·CSV, PNG/JPEG/WebP, DICOM 등의 개인 건강·진료자료를 파일당 5 MiB, 세션당 최대 5개·15 MiB로 받을 수 있습니다. 원본은 세션 메모리에만 보관하고 외부 LLM이나 카탈로그 Action에는 전달하지 않으며, 텍스트 형식만 로컬 추출해 추천 비교에 반영합니다. 스캔·영상은 현재 `accepted_pending_extraction`으로 접수하며 OCR·영상 해석을 수행한 것처럼 표시하지 않습니다. 완료·삭제·만료 때 원본 바이트를 폐기합니다. 익명 데모에서는 실제 개인정보가 없는 가상 테스트 자료만 허용합니다.

결과 화면의 SDC Extraction은 사용자에게 보여주는 설문 결과가 아니라 QuestionnaireResponse를 Observation 등 임상 리소스로 변환하는 연계 기능입니다. 의료인 요약은 질문·답변 목록이 아니라 `Chief Complaint`, `History of Present Illness`, 과거력·수술력·약물·알레르기·가족력·사회력, 우려·추가정보를 영어 의학 표제와 Fact provenance로 정리한 clinician-review draft입니다.
