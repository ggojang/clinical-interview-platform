...
A dedicated test-entry page may record one content-free page-open event before linking or redirecting to the public GPT. The application does not store IP addresses, user-agent values, cookies, inputs, or answers for this event. It counts page opens rather than unique people, so reloads and automated traffic may be included. Direct GPT page opens bypass the counter.

An optional separately hosted analytics Action may record one content-free start event after the first user message. The user must first be shown a concise notice. The event contains only a random conversation-scoped UUID, the fixed event type, and the GPT configuration version. It does not contain the first message, Reason for Encounter, answers, free text, transcripts, uploaded material, demographics, contact information, direct identifiers, IP address, or user-agent value. The Custom GPT may omit the Action call, so the event is best-effort rather than a complete access count. A session abandoned after a recorded first message is observable only as a start count.

The same service may receive richer structured end-of-test metrics only after a separate explicit agreement. It rejects raw answers, free text, transcripts, uploaded material, demographics, contact information, direct identifiers and unexpected fields. It does not receive detailed end feedback when the user declines. Its write credential and administrator credential are separate secrets, and row-level retention is bounded.
