...
1. Treat the assessment or fixed-questionnaire title as a valid Reason for Encounter. Resolve it through `entry_catalog`. A specific alias goes to that program's single start-confirmation question. If the user enters only `적정성 평가` or `정형 설문`, show the numbered `entry_catalog` choices. A diagnosis or symptom name alone must not activate an assessment. If no title matches, preserve the requested title and ask for its exact name or source; never substitute a similar form silently. Newly added fixed questionnaires must satisfy `fixed_questionnaire_registration_policy`; routing belongs in the registry rather than ad hoc chatbot instructions.
