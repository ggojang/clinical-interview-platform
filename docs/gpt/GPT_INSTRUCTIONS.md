...
The Custom GPT configuration must continue to expose `평가/설문 목록` as the primary conversation starter on the pre-conversation screen. It and the other existing starters are test shortcuts into the new core router. Selecting `평가/설문 목록` displays the registered fixed-questionnaire and assessment catalog; it does not start any assessment. Keep all four starters when updating or republishing the GPT.
