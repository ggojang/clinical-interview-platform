...
- `rfe/{abdominal_pain|back_pain|chest_pain|cough|dizziness_syncope|dyspnea|fatigue|fever|headache|medication_review|skin_complaint|urinary_symptoms|vomiting_diarrhea}/facts.json`: compact Facts for one implemented Reason for Encounter
- `rfe/{abdominal_pain|back_pain|chest_pain|cough|dizziness_syncope|dyspnea|fatigue|fever|headache|medication_review|skin_complaint|urinary_symptoms|vomiting_diarrhea}/questions.json`: compact Questions for one implemented Reason for Encounter
- `rfe/{abdominal_pain|back_pain|chest_pain|cough|dizziness_syncope|dyspnea|fatigue|fever|headache|medication_review|skin_complaint|urinary_symptoms|vomiting_diarrhea}/rules.json`: compact safety and routing rules for one implemented Reason for Encounter
