...
| `지역사회건강조사 (Community Health Survey, CHS)` | `official_metadata_registered_rights_review_required` | 아니오 | Create a concept-level crosswalk from the 2025 variable guide to existing Facts, then request written clarification before copying any source-defined item or answer list. |
| `한국의료패널 (Korea Health Panel, KHP)` | `official_metadata_registered_rights_review_required` | 아니오 | Build a concept-only comparison for healthcare use, unmet need, medication and health-literacy Facts; confirm the exact report KOGL marker or obtain written permission before item reuse. |
| `고령화연구패널조사 (Korean Longitudinal Study of Ageing, KLoSA)` | `official_metadata_registered_download_and_rights_review_required` | 아니오 | Complete the declared-use download workflow and rights review, then compare functional, cognition-source, social-support and occupational-context concepts without importing protected scale items. |
...
| 2 | `한국의료패널 (Korea Health Panel, KHP)` | `2025년 한국의료패널 조사표` | source_metadata_registration, concept_and_context_gap_discovery, independent_project_authored_atomic_question_design_with_separate_clinical_evidence | verbatim_appendix_item_embedding, claiming_KHP_form_fidelity, reusing_embedded_third_party_scales_without_owner_review, using_population_associations_as_individual_clinical_rules |
