...
class PainAssessmentTest(unittest.TestCase):
    ...
    def test_palpitations_require_raw_nrs_only_when_chest_pain_is_present(self):
        package = compile_package(profile="palpitations")
        policy = package["interview_completion_policy"]
        conditional = next(
            item for item in policy["conditional_required_facts"]
            if item.get("reason") == "pain_item_activated"
        ...
        self.assertEqual(
            ...
            ["pain.frequency", "palpitations.chest_pain_nrs"],
        )
        self.assertIn(
